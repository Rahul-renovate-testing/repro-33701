...
ISSUE_TYPES_FILTER='-type:Bug -type:Feature -type:Task'

HAS_ISSUES_MISSING_LABELS=false
HAS_ISSUES_MISSING_TYPE=false
...
# Extract the label type from the filter
ISSUE_TYPE=$(echo "$ISSUE_TYPES_FILTER" | cut -d ':' -f 2 | cut -d '-' -f 1)
...
echo "Found issues without labels or type. Please check the issue(s) listed above. Exiting the action."
